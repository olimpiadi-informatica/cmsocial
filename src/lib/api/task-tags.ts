...
export const getTaskTags = cache(
  async (taskName: string, userId: number | undefined): Promise<TaskTag[]> => {
    const canDeleteAny = await hasPermission("tag", "remove-any");
...
export async function addTaskTag(
  ...
  if (!hasPermission("tag", "add")) return msg`Non sei autorizzato`;
...
export async function removeTaskTag(
  ...
  if (!hasPermission("tag", "remove-own")) return msg`Non sei autorizzato`;
  ...
  const deleted = await cmsDb
    ...
    .where(
      and(
        ...
        eq(taskTags.addedBy, userId).if(!hasPermission("tag", "remove-any")),
      ),
    )
    .returning();
