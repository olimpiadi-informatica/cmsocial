...
export const authErrors: Record<keyof typeof auth.$ERROR_CODES | string, MessageDescriptor> = {
  ...
  BANNED_USER: msg`Utente bloccato`,
  COULDNT_UPDATE_YOUR_EMAIL: msg`Email non valida`,
  CREDENTIAL_ACCOUNT_NOT_FOUND: msg`Account non trovato`,
  EMAIL_ALREADY_VERIFIED: msg`Email già verificata`,
  EMAIL_CAN_NOT_BE_UPDATED: msg`L'email non può essere modificata`,
  EMAIL_DOES_NOT_MATCH: msg`L'email non corrisponde a quella dell'account`,
  EMAIL_DOESNT_MATCH: msg`L'email non corrisponde a quella dell'account`,
  EMAIL_IS_THE_SAME: msg`Email uguale alla precedente`,
  EMAIL_MISMATCH: msg`L'email non corrisponde`,
  ...
  INVALID_DISPLAY_USERNAME: msg`Username non valido`,
  ...
  LINKED_ACCOUNT_ALREADY_EXISTS: msg`Account già collegato`,
  ...
  USERNAME_IS_ALREADY_TAKEN: msg`Username non disponibile`,
  USERNAME_IS_ALREADY_TAKEN_PLEASE_TRY_ANOTHER: msg`Username non disponibile`,
  USERNAME_IS_IMMUTABLE: msg`Username non modificabile`,
  ...
  USERNAME_TOO_LONG: msg`Username troppo lungo`,
  USERNAME_TOO_SHORT: msg`Username troppo corto`,
